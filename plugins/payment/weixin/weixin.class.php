...
{    
    ...
    public function  __construct() {   
        ...
        require_once("example/WxPay.MicroPay.php");
    ...
    function getJSAPI($order){
    	...
    	}elseif(stripos($order['order_sn'],'sc') !== false){
            $go_url = U('Mobile/User/scan_pay_success',array('order_id'=>$order['order_id']));
            $back_url = U('Mobile/User/scan_pay',array('store_id'=>$order['store_id']));
        }else{
        ...
        $openId = $_SESSION['openid']; //如果公众号可用就用此openid
        ...
        $input->SetOut_trade_no($order['order_sn'].rand(100000,999999));//$order['order_sn'].time()
...
//        dump(!array_key_exists("appid", $order2));
//        dump(!array_key_exists("prepay_id", $order2));
//        dump($order2['prepay_id'] == "");
//        dump($input);dump($order2);die;
        try{
            $jsApiParameters = $tools->GetJsApiParameters($order2);
        }catch (WxPayException $e){
            exit('<script>alert("'.$e->getMessage().'");location.href="'.$back_url.'";</script>');
        }
        $html = <<<EOF
...
EOF;
     ...
     * @return string
     */
    function get_code2($desc='',$sn='',$total_pay=1,$product_id=0, $config_value='')
    {
        $notify_url = SITE_URL.'/index.php/Home/Payment/notifyUrl/pay_code/weixin';//U('Seller/Card/notifyUrl',[],'',true);//SITE_URL.'/index.php/Home/Payment/notifyUrl/pay_code/weixin'; // 接收微信支付异步通知回调地址，通知url必须为直接可访问的url，不能携带参数。
        //$notify_url = C('site_url').U('Home/Payment/notifyUrl',array('pay_code'=>'weixin')); // 接收微信支付异步通知回调地址，通知url必须为直接可访问的url，不能携带参数。
        //$notify_url = C('site_url')."/index.php?m=Home&c=Payment&a=notifyUrl&pay_code=weixin";
        $input = new WxPayUnifiedOrder();
        $input->SetBody($desc); // 商品描述
        $input->SetAttach("weixin"); // 附加数据，在查询API和支付通知中原样返回，该字段主要用于商户携带订单的自定义数据
        $input->SetOut_trade_no($sn); // 商户系统内部的订单号,32个字符内、可包含字母, 其他说明见商户订单号
        //$input->SetTotal_fee(1);// // 订单总金额，单位为分，详见支付金额
        $input->SetTotal_fee($total_pay*100);//$total_pay*100 // 订单总金额，单位为分，详见支付金额
        $input->SetNotify_url($notify_url); // 接收微信支付异步通知回调地址，通知url必须为直接可访问的url，不能携带参数。
        $input->SetTrade_type("NATIVE"); // 交易类型   取值如下：JSAPI，NATIVE，APP，详细说明见参数规定    NATIVE--原生扫码支付
        $input->SetProduct_id($product_id); // 商品ID trade_type=NATIVE，此参数必传。此id为二维码中包含的商品ID，商户自行定义。
        $notify = new NativePay();
        $result = $notify->GetPayUrl($input); // 获取生成二维码的地址
        //dump($input);dump($result);die;
        $url2 = $result["code_url"];
        return $url2;
        //return '<img alt="模式二扫码支付" src="/index.php?m=Home&c=Index&a=qr_code&data='.urlencode($url2).'" style="width:110px;height:110px;"/>';
    }
    ...
    //刷卡支付测试
    function get_micro_pay($auth_code='',$amount=1,$trade_no=''){
        $input = new WxPayMicroPay();
        $input->SetAuth_code($auth_code);
        $input->SetBody("刷卡测试样例-支付");
        $input->SetTotal_fee($amount);
        $input->SetOut_trade_no(date("YmdHis"));//WxPayConfig::MCHID.
