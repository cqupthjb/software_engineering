...
class ReportController extends BaseController{
    ...
    public function card_rebate_log(){
        ...
        $user['today'] = M('card_rebate_log')->where("confirm_time>$today and status=3 and user_id={$this->_seller['user_id']}")->sum('money');//今日分成
        $user['month'] = M('card_rebate_log')->where("confirm_time>$month and status=3 and user_id={$this->_seller['user_id']}")->sum('money');//本月分成
        $user['total'] = M('card_rebate_log')->where("status=3 and user_id={$this->_seller['user_id']}")->count();//分成总数
        $user['user_money'] = D('card_rebate_log')->where("status=3 and user_id={$this->_seller['user_id']}")->sum('money');//分成总额

        $this->assign('user',$user);
        $sql = "SELECT sum(money) as mo,FROM_UNIXTIME(confirm_time,'%Y-%m-%d') as gap from __PREFIX__card_rebate_log where confirm_time>{$this->begin} and confirm_time<{$this->end} and status=3 and user_id={$this->_seller['user_id']} group by gap";
