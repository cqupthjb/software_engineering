...
use Behavior\RobotCheckBehavior;
...
class StoreController extends BaseController
{
    ...
    public function store_setting()
    {
        ...
        //region 店铺分类
        $store_class = M('store_class')->order('sc_id asc')->select();
        $this->assign('store_class', $store_class);
        //endregion

        //region 店铺折扣选择
        if ($store['is_own_shop'] != 1) {//自营店铺  1：是  0：否
            $discount_list = M('discount')->order('discount_id asc')->select();
    ...
    public function store_description()
    {
        ...
        $descriptions = M('store_description')->where($where)->select();

        foreach ($descriptions as $description) {
            if ($description['status'] == 1) {
                $reviewedDescription = $description;
            } else {
                $reviewingDescription = $description;
            }
        }
        if (!empty($reviewedDescription)) {
            $this->assign('reviewed', $reviewedDescription['content']);
        }

        if (!empty($reviewingDescription)) {
            $this->assign('reviewing', $reviewingDescription['content']);
        }

        if (IS_POST) {
            $where['status'] = 0;
            $reviewingDescription = M('store_description')->where($where)->find();
            if (empty($reviewingDescription)) {
                $where['status'] = 2;
                $reviewingDescription = M('store_description')->where($where)->find();
            }
            ...
            if (empty($reviewingDescription)) {
                ...
                $re = M('store_description')->add($data);
            } else {
                $re = M("store_description")->where($where)->save(array('content' => $content, 'status' => 0));
            }
            if ($re !== false) {
                $this->success("操作成功", U('Store/store_description'));
            } else {
    ...
    {
        try {
            $location = get_latlng_by_ip(get_client_ip());//get_server_ip()
            $this->assign('address', $location['ad_info']);
            $this->assign('lat', $location['location']['lat']);
            $this->assign('lng', $location['location']['lng']);
        } catch (\Exception $e) {
    ...
    public function setting_save()
    {
        ...
        if ($data['act'] == 'update') {
            ...
            if (M('store')->where("store_id=" . STORE_ID)->save($data) !== false) {
    ...
    public function agreement()
    {
        //协议
        $map = array();
        $map['role_id'] = $this->_seller['role_id'];
        $agreement = get_column('distribution_role', $map, 'agreement');
        //当前级别
        $role_name = get_column('distribution_role', $map, 'name');
        //上级
        $map = array();
        ...
        if ($first_leader > 0) {
            ...
            $first_seller_name = get_column('seller', $map, 'seller_name');
        } else {
            $first_seller_name = '钜宏家庭号';
        }

        $this->assign('agreement', $agreement);
        $this->assign('first_seller_name', $first_seller_name);
    ...
    public function scan_order()
    {
        ...
        $url = U('Mobile/User/scan_pay', array('store_id' => $this->_store_id, 'first_leader' => $this->_seller['user_id']), '', true);
    ...
    {
        $where = array('store_id' => STORE_ID, 'type' => 2);
        if (empty($_GET['p'])) {
            $_GET['p'] = 1;
        }
        $list = M('store_description')->where($where)->order('create_time desc')->page($_GET['p'] . ',10')->select();
        $count = M('store_description')->where($where)->count();
        $Page = new \Think\Page($count, 10);
        $show = $Page->show();
        $this->assign('page', $show);
        $this->assign('list', $list);
    ...
    public function upload_video()
    {
        ...
        if ($_FILES['poster']['name']) {
            ...
            if ($info) {
                foreach ($info as $v) {
                    $poster = '/Public/' . $v['savepath'] . $v['savename'];
                }
            }
        } else {
            $this->error('未选择文件或者视频文件过大', U('/Seller/store/store_video'));
            return;
        }
        $where = array('store_id' => STORE_ID, 'status' => '0', 'type' => 2);
        $title = I('title');
        ...
        if (empty($reviewingDescription)) {
            ...
            $file = $reviewingDescription['content'];
            unlink('.' . $file);
            $re = M("store_description")->where($where)->save(array('content' => $content, 'status' => 0));
        }
        if ($re !== false) {
            $this->success("操作成功", U('Store/store_video'));
        } else {
            $this->error("没有修改数据", U('Store/store_video'));
        }
    }
    ...
    private function upload()
    {
        ...
        $upload->savePath = 'upload/video/poster/'; // 设置附件上传（子）目录
        $upload->autoSub = true;
    ...
    {
        $id = 'LTAIz8rEfxzCc2Gq';          // 请填写您的AccessKeyId。
        $key = 'uNYCBkBoOQT26MjI4yUZl2EDNZQtfu';     // 请填写您的AccessKeySecret。。
        $host = 'http://jhvideo.oss-cn-hangzhou.aliyuncs.com';
        $dir = 'store_video/';          // 用户上传文件时指定的前缀。
        $now = time();
