...
class AdminController extends BaseController {
            ...
            if(!empty($seller_name) && !empty($password)){
				...
				if($seller){
					...
					if($user){
						...
						#region 判断店铺状态
						$store_state = get_column('store',array('user_id'=>$user['user_id']),'store_state');

						if ($store_state!= 1){
						    if ($store_state == 2)
                                exit(json_encode(array('status'=>0,'msg'=>'店铺还未审核')));
						    else
                                exit(json_encode(array('status'=>0,'msg'=>'店铺已关闭')));
                        }
                        #endregion
    ...
    public function role_info(){
    	...
    	$menu_list2 = getMenuList($this->_seller['role_id']);
