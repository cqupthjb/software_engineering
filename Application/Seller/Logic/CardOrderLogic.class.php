...
class CardOrderLogic extends RelationModel
{
    ...
    public function getCardOrderButton($order){
        /*
         *  操作按钮汇总 ：付款、设为未付款、确认、取消确认、无效、去发货、确认收货、申请退货
         *
         */
    ...
    public function orderProcessHandle($order_id,$act,$store_id = 0){
    	...
    	switch ($act){
    		case 'pay': //付款
                return false;
                $order_sn = M('card_order')->where("order_id = $order_id")->getField("order_sn");
    	...
    	return false;
    	return M('card_order')->where("order_id = $order_id and store_id = $store_id")->save($updata);//改变订单状态
    }
    ...
    function order_pay_cancel($order_id)
    {
    	...
    	// 找出对应的订单
    	$order = M('card_order')->where("order_id = $order_id")->find();
    	// 增加对应商品的库存
    	...
    	//分销设置
    	M('card_rebate_log')->where("order_id = {$order['order_id']}")->save(array('status'=>0));
    }
    ...
    public function deliveryHandle($data,$store_id=0){
		...
		foreach ($orderGoods as $k=>$v){
			...
			if($v['is_send'] == 0 && in_array($v['rec_id'],$selectgoods)){
				$res['is_send'] = 1;
				$res['delivery_id'] = $did;
                 //$r = M('card_suborder')->where("rec_id={$v['rec_id']}  and store_id = $store_id")->save($res);//改变订单商品发货状态
                 $r = M('card_suborder')->where("rec_id={$v['rec_id']}")->save($res);//改变订单商品发货状态
				$is_delivery++;
			}
		}
		...
		$s = $this->orderActionLog($order['order_id'],'delivery',$data['note'],$seller_id);//操作日志
		return $s && $r;
    }
    ...
    function delOrder($order_id,$store_id){
    	$a = M('card_order')->where(array('order_id'=>$order_id,'store_id'=>$store_id))->delete();
    	if ($a !== false){
            $b = M('card_suborder')->where(array('order_id'=>$order_id))->delete();
        }else{
    	    $b = false;
        }
    	return $a && $b;
