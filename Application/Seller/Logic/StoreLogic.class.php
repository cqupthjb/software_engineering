...
{    
     ...
     * @param int $role_id      店铺角色id  1：省级  2：地级   3：县级    4：经销商
     * @param array $_seller    当前添加店铺人员的信息
     * @return bool
     */
    public function addStore($store=[],$role_id=0,$_seller=[])
    {
        //添加前台登陆账号
        $model = new Model();
        $model->startTrans();
        ...
        $user = array($utype=>$store['user_name'],'password'=>encrypt($store['password']),'reg_time'=>time(),'role_id'=>$role_id,'first_leader'=>$_seller['user_id']);
        //if (in_array($_seller['role_id'],array(4,5,0))) $user['is_distribut'] = 1;
        #region 默认为分销商
        $user['is_distribut'] = 1; //如果不是自营店铺 默认为分销商 1
        $user['iskaidian'] = 1;
        #endregion
        $user_id = $model->table($db_prefix.'users')->add($user);
        $store['user_id'] = $user_id;
        $store['role_id'] = $role_id;
        $store['first_leader'] = $_seller['store_id'];
        ...
        //添加店铺管理员
        $seller = array('seller_name'=>$store['seller_name'],'store_id'=>$store_id,'user_id'=>$user_id,'is_admin'=>1,'role_id'=>$role_id,'add_time'=>time());
        ...
        if($user_id && $store_id && $seller_id){
            ...
            //店铺操作日志
            store_log($store_id,$store['first_leader'],'后台添加商户：'.$store['store_name']);
            return true;
        }else{
            $model->rollback();
            return false;
