...
class GoodsLogic extends RelationModel
{
   ...
   {
//        $c1 = M('Comment')->where("goods_id = $goods_id and parent_id = 0 and  ceil(goods_rank) in(4,5)")->count(); // 好评
//        $c2 = M('Comment')->where("goods_id = $goods_id and parent_id = 0 and  ceil(goods_rank) in(3)")->count(); // 中评
//        $c3 = M('Comment')->where("goods_id = $goods_id and parent_id = 0 and  ceil(goods_rank) in(1,2)")->count(); // 差评
       $c1 = M('Comment')->where("goods_id = $goods_id  and  ceil(goods_rank) in(4,5) and deleted = 0 and is_show=1")->count(); // 好评
       $c2 = M('Comment')->where("goods_id = $goods_id  and  ceil(goods_rank) in(3) and deleted = 0 and is_show=1")->count(); // 中评
       $c3 = M('Comment')->where("goods_id = $goods_id  and  ceil(goods_rank) in(1,2) and deleted = 0 and is_show=1")->count(); // 差评
       $c4 = M('Comment')->where("goods_id = $goods_id  and  img !='' and img NOT LIKE 'N;%'")->count(); // 晒图
...
function getGoodsIdByAttr($attr)
{
   ...
   if($c > 0)
    ...
    function getGoodsDispatching($goods_id,$region_id)
    {
        ...
        foreach($goods_shipping as $k=>$v){
            $goods_shipping[$k]['freight'] = $cart_logic->cart_freight2($goods_shipping[$k]['shipping_code'],0,0,$goods_shipping[$k]['region_id'],$goods['weight'],$goods['store_id']);
            $goods_shipping[$k]['shipping_name'] = M('plugin')->where(array('type'=>'shipping','code'=>$goods_shipping[$k]['shipping_code']))->getField('name');
        }
        $return_data = array(
            'status'=>1,
