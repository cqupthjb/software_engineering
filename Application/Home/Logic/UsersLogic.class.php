...
class UsersLogic extends RelationModel
{
     ...
     * @param bool $is_wx      是否是微信端
     * @return array
     */
    public function thirdLogin($data=array(),$is_wx = true){
        ...
        if(!$user){
            //账户不存在 注册一个
            $map['wxcountry'] = $data['country']?$data['country']:"";
            $map['wxprovince'] = $data['province']?$data['province']:"";
            $map['wxcity'] =  $data['city']?$data['city']:"";

            $map['password'] = '';
            if ($is_wx){
                $map['openid'] = $openid;
            }else{
                $map['app_openid'] = $openid;
            }
            if (isset($data['unionid']))
                $map['unionid'] = $data['unionid'];
            $map['nickname'] = preg_replace('/[\x{10000}-\x{10FFFF}]/u', '', $data['nickname']);
            $map['reg_time'] = time();
            $map['oauth'] = $oauth;
...
//                $map['third_leader'] = $first_leader['second_leader']; // 第二级推荐人
        ...
        }else
        {
            ...
            $user['token'] = md5(time().mt_rand(1,999999999));
            $data = array('token'=>$user['token'],'','last_login'=>time());
            if (!$is_wx && empty($user['app_openid'])){
                $user['app_openid'] = $openid;
                $data['app_openid'] = $openid;
            }else if($is_wx && empty($user['openid'])){
                $user['openid'] = $openid;
                $data['openid'] = $openid;
            }
    ...
    public function reg($username,$password,$password2){
        ...
        //判断密码格式是否正确 by wangjing 2016-11-16
		if (!is_password($password)){
			return array('status'=>-1,'msg'=>'密码格式错误','result'=>'');
		}
        //验证两次密码是否匹配
...
//        $type = I('get.type',0);
     ...
     * @param $user_id  用户id
     */
    public function get_store_collect($user_id){
        $count = M('store_collect')->where(array('user_id'=>$user_id))->count();
        $page = new Page($count,10);
        $show = $page->show();
        //获取我的收藏列表
        $sql = "SELECT c.log_id,c.add_time,g.store_id,g.store_name,g.store_logo FROM __PREFIX__store_collect c ".
            "inner JOIN __PREFIX__store g ON g.store_id = c.store_id ".
            "WHERE c.user_id = ".$user_id.
            " ORDER BY c.add_time DESC LIMIT {$page->firstRow},{$page->listRows}";
        $result = M('store_collect')->query($sql);
        $return['status'] = 3;
        $return['msg'] = '获取成功';
        $return['result'] = $result;
        $return['show'] = $show;
...
//        	}
            ...
            $sql =" select o.add_time,o.order_sn,og.order_id,og.goods_id,og.goods_name,og.goods_price,og.goods_num,o.store_id from __PREFIX__order_goods as og
            left join __PREFIX__order as o on o.order_id = og.order_id  where o.user_id = $user_id and og.is_send = 1
     ...
     * @return array
     */
    public function add_address($user_id,$address_id=0,$post){
        if($address_id == 0)
        {
    ...
    public function cancel_order($user_id,$order_id){
        ...
        //有充值卡支付金额
        if ($order['card_money']>0 || $order['active_card_money']>0){
            unset($map);
            $map['user_id'] = array('eq',$user_id);
            $temp = $order['card_money']+$order['active_card_money'];
            M('Users')->where($map)->setInc('card_money',$temp);
        }
        if ($order['active_card_money']>0){
            unset($map);
            $map['user_id'] = array('eq',$user_id);
            $user_info = M('Users')->where($map)->find();
            $temp = $user_info['active_card_money'] + $order['active_card_money'];
            if ($temp>30){
                $temp =30;
            }
    ...
    public function send_validate_code($sender,$type){
    	...
    	/*if(!empty($send) && $send['time'] > time() && $send['sender'] == $sender){
    		//在有效期范围内 相同号码不再发送
    		return array('status'=>-1,'msg'=>'规定时间内,不要重复发送验证码');
    	}*/
    	$code =  mt_rand(1000,9999);
    	if($type == 'email'){
    		//检查是否邮箱格式
    		if(!check_email($sender))
                return array('status'=>-1,'msg'=>'邮箱码格式有误');
    		else
