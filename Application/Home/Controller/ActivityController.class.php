...
class ActivityController extends BaseController {
    ...
    public function  sendTestMsg(){
        echo json_encode(send_wx_msg(I("openId"),I("msg")));
    }
    ...
    */ 
    ...
    {
    	$count =  M('GroupBuy')->where(time()." >= start_time and ".time()." <= end_time and status=1")->count();// 查询满足要求的总记录数
    	$Page = new \Think\Page($count,20);// 实例化分页类 传入总记录数和每页显示的记录数  	
    	$show = $Page->show();// 分页显示输出
    	$this->assign('page',$show);// 赋值分页输出
        $list = M('GroupBuy')->where(time()." >= start_time and ".time()." <= end_time and status=1")->limit($Page->firstRow.','.$Page->listRows)->select(); // 找出这个商品        
