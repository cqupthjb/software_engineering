...
class ApiController extends Controller {
    ...
    public function get_map_area(){
        ...
        $key = 'BJVBZ-4CMWG-JQFQC-I2P6E-JXQX6-IIBGW';
        ...
        //当前区域与地址
        $city_name = $data['result']['address_component']['district'];//区域名称
        $city_address = $data['result']['address'];//当前地址

        if(!empty($city_address)){
            cookie('address',$city_address);
            $res = array('status'=>1,'msg'=>'定位成功','address'=>$city_address,'lat'=>$lat,'lng'=>$lng);
        }else{
            $res = array('status'=>0,'msg'=>'定位失败');
        }
