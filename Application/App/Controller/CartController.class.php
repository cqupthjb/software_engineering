...
class CartController extends MobileBaseController {
    ...
    public function cart2()
    {
        ...
        if(empty($address)){
            header("Location: ".U('User/add_address',array('source'=>'cart2')));
        }else{
...
//            }
//            $result['cartList'][$k]['value'] = $rebate['value'];
//            $result['cartList'][$k]['money'] =  $rebate['money'];
//        }
        //endregion
    ...
    public function cart3(){
        ...
        $card_money =  I("card_money",0); //  使用充值金额 //新添加
        ...
        $result = calculate_price($this->user_id,$order_goods,$shipping_code,0,$address[province],$address[city],$address[district],$pay_points,$user_money,$coupon_id,$couponCode,$card_money);
        ...
        $car_price = array(
            ...
            'card_balance'      => $result['result']['card_money'], // 使用用户充值金额//新加的东西
    ...
    public function cart4(){
        ...
        {                       
            ...
            if($sum_order_amount == 0){                
                $order_order_list = U("App/User/order_list");
        ...
        else
        {
            ...
            if($order['pay_status'] == 1){
                $order_detail_url = U("App/User/order_detail",array('id'=>$order_id));
        ...
        if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger'))
            ...
            $_paymentList = M('Plugin')->where("`type`='payment' and status = 1 and code in('weixin','cod')")->find();
            $paymentList = M('Plugin')->where("`type`='payment' and status = 1 and  scene in(1)")->select();
            array_unshift($paymentList,$_paymentList);
        }
        ...
        $this->assign('pay_url',U('Api/Wxpay/dopay','','',true));
        $this->display();                   
    }
    ...
    */
    ...
    {
        $post_goods_num = I("goods_num",0); // goods_num 购物车商品数量
 ...
 */
    ...
    public function ajaxDelCart()
    {
        ...
        $count = M("Cart")->where(array('user_id'=>$this->_uid))->count();
        $return_arr = array('status'=>1,'msg'=>'删除成功','result'=>'','count'=>$count); // 返回结果状态
        exit(json_encode($return_arr));
    }
    ...
    public function get_cart_num(){
        $where = " session_id = '$this->session_id' "; // 默认按照 session_id 查询
        $this->user_id && $where = " user_id = ".$this->user_id; // 如果这个用户已经等了则按照用户id查询
        $num = M('Cart')->where($where)->sum("goods_num");
        $num = $num ? $num : 0;
        $this->ajaxReturn(array('num'=>$num));
    }

    //根据id获取订单  //方案二可用 改成public  有bug 不建议使用 请使用apicloud 的wxPay方案一
    protected function get_order_by_id(){
        try{
            $order_id = I('id',0);
            if (empty($order_id))
                throw new Exception('参数错误',401);
            $order = M('order')->where(array('order_id'=>$order_id))->field('order_id,order_sn,order_status,pay_status,order_amount,user_id')->find();
            if (empty($order))
                throw new Exception('该订单不存在',402);
            if ($order['pay_status'] == 1)
                throw new Exception('该订单已支付',403);
            $user = M('users')->where(array('user_id'=>$order['user_id']))->find();
            if (empty($user))
                throw new Exception('下订单的用户不存在',404);
            if (empty($user['app_openid']))
                throw new Exception('请先绑定微信');
            $order['openId'] = $user['app_openid'];//openid
            $order['description'] = M('order_goods')->where(array('order_id'=>$order_id))->getField('goods_name').'等';//描述
            $order['spbillCreateIP'] = get_client_ip();//客户端ip
            $order['feeType'] = 'CNY';//货币类型 ：CNY：人名币
            $order['timeStart'] = date('YmdHis',time());//开始时间
            $order['timeExpire'] = date('YmdHis',time()+60*5);//结束时间 比开始时间 >=5 分钟
            $order['tradeNo'] = $order['order_sn'].rand(100000,999999);//微信平台订单号
            //$order['totalFee'] = $order['order_amount'] * 100;//支付金额
            $order['totalFee'] = 1;//支付金额

            $res = array('status'=>200,'msg'=>'success','data'=>$order);
        }catch (Exception $e){
    ...
    //有bug//接口可直接调用  //方案二可用 改成public  有bug 不建议使用 请使用apicloud 的wxPay方案一
    protected function app_pay(){
        try{
            $timeStart = I('timeStart',0);
            if (empty($timeStart))
                throw new Exception('参数缺省',401);
//            if (strtotime($timeStart)<time())
//                throw new Exception('已超时',402);
            $order_id = I('id',0);
            if (empty($order_id))
                throw new Exception('参数缺省',401);
            $type = I('type','weixin');
            $map = array('order_id'=>$order_id);
            M('order')->where($map)->setField(array('pay_code'=>$type));
            $order_sn = M('order')->where($map)->getField('order_sn');
            update_pay_status($order_sn);
            $res = array('status'=>200,'msg'=>'支付成功');
        }catch(Exception $e){
            $res = array('status'=>$e->getCode(),'msg'=>$e->getMessage());
        }
        $this->ajaxReturn($res);
