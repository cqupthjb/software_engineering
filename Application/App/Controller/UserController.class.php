...
class UserController extends MobileBaseController
{
    ...
    */
    public function _initialize()
    {
        ...
        $nologin = array(
            'login', 'pop_login', 'do_login', 'logout', 'verify', 'set_pwd', 'finished','get_about_link',
            'verifyHandle', 'reg', 'send_sms_reg_code', 'find_pwd', 'check_validate_code',
            'forget_pwd', 'check_captcha', 'check_username', 'send_validate_code', 'express',
        );
        if (!$this->user_id && empty($this->user_id)&& !in_array(ACTION_NAME, $nologin)) {
    ...
    public function login()
    {
        ...
        $referurl = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : U("App/User/index");
    ...
    public function add_address()
    {
        if (IS_POST) {
            ...
            elseif ($_POST['source'] == 'cart2') {
                header('Location:' . U('/App/Cart/cart2', array('address_id' => $data['result'])));
                exit;
            }

            $this->success($data['msg'], U('/App/User/address_list'));
            exit();
        }
    ...
    public function edit_address()
    {
        ...
        if (IS_POST) {
            ...
            if ($_POST['source'] == 'cart2') {
                header('Location:' . U('/App/Cart/cart2', array('address_id' => $id)));
                exit;
            } else
                $this->success($data['msg'], U('/App/User/address_list'));
            exit();
    ...
    public function set_default()
    {
        ...
        if ($source == 'cart2') {
            header("Location:" . U('Cart/cart2'));
            exit;
        } else {
            header("Location:" . U('User/address_list'));
        }
    }
    ...
    public function add_comment()
    {
        if (IS_POST) {
            ...
            if ($row[status] == 1) {
                $this->success('评论成功', U('/App/Goods/goodsInfo', array('id' => $add['goods_id'])));
                exit();
            } else {
                $this->error($row[msg]);
            }
...
//        $this->assign('message', $message);
    ...
    public function get_about_link(){
        //关于我们
        $about_type = get_column('config',array('inc_type'=>'about_me','name'=>'about_type'),'value');
        if ($about_type){
            $about_link = get_column('config',array('inc_type'=>'about_me','name'=>'about_link'),'value');
        }else{
            $about_link = U('App/User/about_me','','',true);
        }
        $this->ajaxReturn(array('status'=>1,'msg'=>'success','link'=>$about_link));
    }
    ...
    //region 绑定微信  App bind_wx
    public function bind_wx(){
        try{
            $info = $this->_check_wx_info();//验证及获取数据
            $data = array();
            $data['oauth'] = 'weixin';
            $data['unionid'] = $info['unionid'];
            $data['app_openid'] = $info['openid'];
            //$data['nickname'] = $info['nickname'] ? preg_replace('/[\x{10000}-\x{10FFFF}]/u', '', $data['nickname']) : $this->_user['nickname'];
            //$data['head_pic'] = $data['head_pic'];
            //$data['sex'] = $data['sex'] === null ? 0 :  $data['sex'];
            //$data['token'] = md5(time().mt_rand(1,99999));
            $res = M('users')->where(array('user_id'=>$this->_uid))->save($data);
            if ($res == false){
                throw new Exception('绑定失败');
            }
            $res = array('status'=>1,'msg'=>'绑定成功','openid'=>$info['openid'],'unionid'=>$info['unionid']);
        }catch(Exception $e){
    ...
    //数据验证
    protected function _check_wx_info(){
        $info = I('post.');
        if (empty($info))
            throw new Exception('获取数据失败');
        if (empty($info['openid']) || empty($info['unionid']))
            throw new Exception('获取数据失败.');
        $wx_user = M('users')->where(array('unionid'=>$info['unionid']))->find();
        if (!empty($wx_user)){
            if (!empty($wx_user['app_openid'])){ //已绑定微信
                if ($wx_user['user_id'] != $this->_uid){
                    throw new Exception('该微信已被绑定');
                }else{
                    throw new Exception('您已绑定微信');
                }
            }else{ //未绑定微信
                if ($wx_user['user_id'] != $this->_uid){
                    throw new Exception('该微信号与微信公众号登录信息不符');
                }
            }
        }
        return $info;
