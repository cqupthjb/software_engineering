...
class StoreController extends MobileBaseController {
	...
	public function about(){
        ...
        if ($this->store['is_own_shop'] ==1){ //自营
            $ShareLink = U('Store/index', array('store_id' => $this->store['store_id'], ), '', true);// 'first_leader' => $this->_seller['user_id']
        }else{//入驻店铺（实体店）
            $ShareLink = U('Store/index', array('store_id' => $this->store['store_id'], 'first_leader' => $this->store['user_id']), '', true);
        }
        ...
        $this->assign('description',$this->store['description']);
