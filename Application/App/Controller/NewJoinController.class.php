...
class NewJoinController extends MobileBaseController
{
    ...
    public function setting_info(){
        ...
        if (IS_POST) {
            try{
                ...
                $this->success('绑定成功,请进入商户管理后台进行系列管理',U('App/index/index'));
            }catch(Exception $e){
                if ($e->getCode()==0){
                    $this->error($e->getMessage());die;
                }
                $this->error($e->getMessage(),U('App/index/index'));
            }
        }
    ...
    protected function _check_data(){
        ...
        $userLogic = new UsersLogic();
        $check_code = $userLogic->sms_code_verify($data['mobile'], $data['mobile_code'], $this->session_id);
...
//            else
        ...
        if ($max_num>0 && $count_store >= $max_num){ //数量为0则不限制
            $this->error('下线数量已满'.$max_num.'个',U('Index/index'));die;
        }
        #endregion
    }
    ...
    protected function _settled($data){
        ...
        if ($next_role_id > 0){
            ...
            $store = array(
                ...
                'store_state'   =>  2,//0关闭，1开启，2审核中  因为是二维码注册，以防再次分享注册
            ...
            $storeLogic = new \App\Logic\StoreLogic();
            if($storeLogic->addStore($store,$next_role_id)){
                ...
                $this->success('店铺入驻成功,请前往pc端商户管理后台进行操作',U('App/Index/index'));die;
            }else{
    ...
    public function test(){
        /*$user = M('users')->where(array('user_id'=>I('uid')))->find();
        dump($leader_list = $this->_find_leader($user,[]));*/
        $this->_send_sms(I('uid'));
    }
    ...
    private function _send_msg_to_superior($first_leader,$uid){
        ...
        if ($first_leader == 0){
            ...
            $content .= '.上级：'.$this->_send_content($first_leader_user,'');
            M('test_msg')->add(array('msg'=>serialize($content)));
    ...
    private function _send_msg_to_level($user,$content){
        ...
        if (!empty($user)){ //上级为非平台
            //发送本次短信
            send_sms('13101362529',$content);//$user['mobile']
