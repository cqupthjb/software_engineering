...
class CartLogic extends RelationModel
{
    ...
    {   ini_set('max_execution_time', '0');//解除时间限制
        ...
        foreach($car_price['store_order_amount'] as $k => $v)
        {
            ...
            // 扣除用户的充值金额
            if ($is_own_shop){ //自营店铺
                if ($car_price['card_balance'] > 0)
                    M('Users')->where("user_id = $user_id")->setDec('card_money',$car_price['card_balance']);
            }
            ...
            if ($is_own_shop){ //自营店铺
                $data4['card_money'] = -$car_price['card_balance'];
                $data5 = $data4;
            ...
            /*if (!empty($user['reg_id'])){
                $title = '订单消息';
                $content = "你刚刚下了一笔订单:{$order['order_sn']} 尽快支付,过期失效!";
                send_jpush_msg($user['reg_id'],$title,$content,'order');
            }*/
