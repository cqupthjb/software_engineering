...
class StoreModel extends Model{
     ...
     * @return mixed
     */
    public function getStreetListLBS($sc_id,$p=1,$item=10,$order_by='',$keywords='',$lat=0,$lng=0)
    {
        $model = M('');
        $store_where = array();
        $store_where['s.role_id'] = array('in',array(0,5));
        ...
        if (!empty($keywords)){
            $store_where['s.store_name'] = array('like',"%$keywords%");
        }

        //排序
        $order = " (ABS(s.store_lng - '{$lng}') +  ABS(s.store_lng - '{$lat}') ) asc ";
        if ($order_by){
...
//            'is_hot'=>1,
        ...
        $res['goods_list'] = $goods_model->field('goods_id,goods_name,shop_price,original_img')->where($goods_where)->limit($limit)->order('sort desc')->select();
        $count_where = array(
            'goods_state'=>1,
            'store_id'=>$store_id
        );
