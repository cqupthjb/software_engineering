...
class BaseController extends Controller
{
    ...
    public function checkToken()
    {
        ...
        $check_arr = array(
            ...
            'user' => array('getcollectstoredata', 'getusercollectstore', 'userinfo', 'updateuserinfo', 'password', 'getaddresslist', 'addaddress', 'del_address', 'setdefaultaddress', 'getcouponlist', 'getgoodscollect', 'getorderlist', 'getorderdetail', 'cancelorder', 'orderconfirm', 'add_comment', 'account', 'return_goods_list', 'return_goods_info', 'return_goods_status', 'return_goods'), // 需要验证登录的方法
            'cart' => array('cart2', 'cart3'), // 需要验证登录的方法
            'goods' => array('collectgoods'), // 需要验证登录的方法
            'store' => array('collectstoreorno'),// 需要验证登录的方法
        );
        ...
        if (in_array($controller_name, array_keys($check_arr)) && in_array($action_name, $check_arr[$controller_name])) {
            if (empty($this->token))
                $this->ajaxReturn(array('status' => -100, 'msg' => '必须传递token', 'result' => ''));

            $this->user = M('users')->where("token = '{$this->token}'")->find();
            if (empty($this->user))
                $this->ajaxReturn(array('status' => -101, 'msg' => 'token错误', 'result' => ''));
            // 登录超过72分钟 则为登录超时 需要重新登录.  //这个时间可以自己设置 可以设置为 20分钟
            if (time() - $this->user['last_login'] > 3600000)   //3600
                $this->ajaxReturn(array('status' => -102, 'msg' => '登录超时,请重新登录!!!', 'result' => (time() - $this->user['last_login'])));
