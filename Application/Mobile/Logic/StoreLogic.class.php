...
{    
    ...
    public function addStore($store=[],$role_id=0)
    {
		...
		//添加店铺管理员
		$seller = array('seller_name'=>$store['seller_name'],'store_id'=>$store_id,'user_id'=>$store['user_id'],'is_admin'=>1,'role_id'=>$role_id,'add_time'=>time());
		$seller_id = $model->table($db_prefix.'seller')->add($seller);
		if($store['user_id'] && $store_id && $seller_id){
			$model->commit();
            ...
            //店铺操作日志
            store_log($store_id,$store['first_leader'],'二维码商户入驻：'.$store['store_name']);
			return true;
		}else{
			$model->rollback();
			return false;
