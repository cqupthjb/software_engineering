...
class CartLogic extends RelationModel
{
    ...
    {       
        ...
        // 商品参与促销
        $promote_price = $this->get_goods_promote_price($goods_id,1); // 获取促销活动价
        if($promote_price > 0)
        {
            $price = $promote_price; // 如果该商品参与了促销活动 则使用促销活动价    
            $activity_type = 1; // 限时抢购商品
        }

        // 获取商品团购价
        $group_buy_price = $this->get_group_buy_price($goods_id,1); // 获取促销活动价
        if($group_buy_price > 0)
        {
            $price = $group_buy_price; // 如果该商品参与了团购活动 则使用团购活动价
            $activity_type = 2; // 限时抢购商品
        }
...
function cart_freight2($shipping_code,$province,$city,$district,$weight)
{
   ...
   $weight = ceil($weight / 1000); // 按千取整 不够1000 按1000
   ...
   return $freight;  
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price)
    {
        ...
        $data = array(
                ...
                'shipping_code'    =>$shipping_code?$shipping_code:'0',//'物流编号',
        ...
        if($coupon_id>0){
        	...
        	if($coupon){
        		...
        		$data3['use_time'] = time();
        		M('coupon')->where(array('id'=>$coupon['cid']))->setInc('use_num');
    ...
    * $mode 0  返回数组形式  1 直接返回result
    */
   public function get_goods_promote_price($goods_id,$mode=0)
   {
       $goods = M('Goods')->where("goods_id = $goods_id")->find(); // 找出这个商品        
       $price = 0;
       if($goods[is_promote] == 1 && time() > $goods[promote_start_time] && time() < $goods[promote_end_time])
       {
           $price = $goods[promote_price];
       } 
       
        if($mode == 1) return $price;
        return array('status'=>1,'msg'=>'','result'=>$price);       
   }    
   /**
    * 获取商品团购价
    * 如果商品没有团购活动 则返回 0
