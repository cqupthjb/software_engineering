...
class CartController extends MobileBaseController {
    ...
    public function  __construct() {   
        ...
        if(session('?user'))
        {
            ...
            $user['use_card_money'] = $user['card_money'] - $user['active_card_money'];
        	$this->user = $user;
    ...
    function ajaxAddCart()
    {
        ...
        if ($goods['min_quantity'] > 0 && $goods_num < $goods['min_quantity']){
            exit(json_encode(array('status'=>-1,'msg'=>'该商品'.$goods['min_quantity'].'件起卖','result'=>'')));
        }
...
//        }
    ...
    public function cart3(){
        ...
        $active_card_money = I("active_card_money",0); //  使用的15,30,100元充值金额
        //$card_money +=$active_card_money;
        ...
        $result = calculate_price($this->user_id,$order_goods,$shipping_code,0,$address[province],$address[city],$address[district],$pay_points,$user_money,$coupon_id,$couponCode,$card_money,$active_card_money);
        ...
        $car_price = array(
            ...
            'active_card_money'=>$result['result']['active_card_money'],//使用的特殊充值卡金额
            'card_money'=>$result['result']['card_money']+$result['result']['active_card_money'],
        );
        // 提交订单
