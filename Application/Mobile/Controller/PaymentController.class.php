...
class PaymentController extends MobileBaseController
{
     ...
     * 支付批发商入驻费用
     */
    public function payWholesalerFee(){
        //手机端在线充值
        C('TOKEN_ON', false); // 关闭 TOKEN_ON
        header("Content-type:text/html;charset=utf-8");
        $user = session('user');
        ...
        $data['order_sn'] = 'fee' . get_rand_str(15, 0, 1);
        $data['create_time'] = time();
        $order_id = M('wholesaler_fee')->add($data);

        if ($order_id) {
            $order = M('wholesaler_fee')->where("id = $order_id")->find();
            if (is_array($order) && $order['status'] == 0) {
                $order['order_amount'] = $order['fee']; //$order['account']; //折扣金额
                $pay_radio = $_REQUEST['pay_radio'];
                $config_value = parse_url_param($pay_radio); // 类似于 pay_code=alipay&bank_code=CCB-DEBIT 参数
                $payment_arr = M('Plugin')->where("`type` = 'payment'")->getField("code,name");
                M('wholesaler_fee')->where("id = $order_id")->save(array('pay_code' => $this->pay_code, 'pay_name' => $payment_arr[$this->pay_code]));
                //微信JS支付
                if ($this->pay_code == 'weixin' && $_SESSION['openid'] && strstr($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')) {
                    $code_str = $this->payment->getJSAPI($order);
                    exit($code_str);
...
//        $this->display('recharge'); //分跳转 和不 跳转
    ...
    public function get_scan_pay()
    {
        ...
        if (!M()->autoCheckToken(I('post.'))){
            $this->error('请刷新重试',U('Mobile/User/scan_pay',array('store_id'=>$store['store_id'])));die;
        }
