...
class OrderLogic extends RelationModel
{
    ...
    public function getScanOrderList($condition,$order='',$start=0,$page_size=20){
        $res = M('scan_order')->where($condition)->limit("$start,$page_size")->order($order)->select();
        return $res;
    }
    ...
    public function getOrderGoods($order_id){
        ...
        $field = 'o.* , g.* ,(o.goods_num * o.member_goods_price) AS goods_total';
        $join = array('LEFT JOIN __GOODS__ AS g ON g.goods_id = o.goods_id');
        $alias = 'o';
        $res = M('order_goods')->alias($alias)->join($join)->where($map)->field($field)->select();
        return $res;
    }
