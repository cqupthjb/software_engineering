...
{    
     ...
     * @param int $role_id      店铺角色id  1：省级  2：地级   3：县级    4：经销商  5:实体店铺
     * @return bool
     */
    public function addStore($store=[],$role_id=0)
    {
    	//添加前台登陆账号
    	$model = new Model();
    	$model->startTrans();
		...
		$user = array($utype=>$store['user_name'],'password'=>encrypt($store['password']),'reg_time'=>time(),'role_id'=>$role_id);
		#region 是否为分销商
		...
		if (get_count('store_extend',array('store_id'=>$store_id)) == 0)
		...
		//添加店铺管理员
		$seller = array('seller_name'=>$store['seller_name'],'store_id'=>$store_id,'user_id'=>$user_id,'is_admin'=>1,'role_id'=>$role_id);
		...
		if($user_id && $store_id && $seller_id){
			...
			if ($store['is_own_shop'] == 0){
                //店铺操作日志
                store_log($store_id,0,'后台添加商户：'.$store['store_name']);
            }
			return true;
		}else{
			$model->rollback();
    ...
    public function auto_transfer($store_id){
        // 确认收货多少天后 自动结算给 商家
        $today_time = time();
        $auto_transfer_date = 0;//tpCache('shopping.auto_transfer_date');
