...
class FinanceController extends BaseController {
    ...
    public function withdrawals()
    {
        ...
        $create_time2 = $create_time  ? $create_time  : date('Y-m-d',strtotime('-1 year')).' - '.date('Y-m-d',strtotime('+1 day'));
        $create_time3 = explode(' - ',$create_time2);
        $where['w.create_time'] =  array(array('gt', strtotime(strtotime($create_time3[0]))));
    ...
    public function editWithdrawals(){
        ...
        if(IS_POST)
        {
                ...
                $model->save();                               
                $this->success("操作成功!",U('withdrawals'),3);//remittance
                exit;
        }       
       if($user['nickname'])        
    ...
    public function queryPay(){
        $id = I('id');
        $type = I('type');
        if (!in_array($type,array('withdrawals','store_withdrawals'))){
            $this->error("查询条件错误");
            exit;
        }
        $model = M($type);
        ...
        $wechatPayServer = new WechatPayServer($data);
        try{
            $res = $wechatPayServer->searchPay($withdrawals['order_sn']);
            if ($res['result_code'] != 'SUCCESS'){
                throw new Exception($res['err_code_des']);
            }
            switch ($res['status']){
                case 'PROCESSING':$res['status_info'] = '处理中'; break;
                case 'SUCCESS':$res['status_info'] = '付款成功'; break;
                case 'FAILED':$res['status_info'] = '付款失败'; break;
            }
            $this->assign('info',$res);
            //$res = array('status'=>1,'msg'=>'查询成功','data'=>$res);
        }catch (Exception $e){
