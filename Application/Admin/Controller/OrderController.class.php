...
class OrderController extends BaseController
{
    ...
    public function ajaxindex()
    {
        ...
        if ($store_name) { //"store_name like '%$store_name%'"
            $store_id_arr = M('store')->where(array('store_name'=>array( 'like' ,"%$store_name%")))->getField('store_id', true);
            if ($store_id_arr) {
    ...
    */
    ...
    public function export_scan_order()
    {
       ...
       $where = '';
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="100">日期</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="100">店铺</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">订单金额</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">实际支付</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">支付方式</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">支付状态</td>';
        ...
        foreach ($orderList as $k => $val) {
            ...
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $val['create_time'] . ' </td>';
            $strTable .= '<td style="text-align:center;font-size:12px;">&nbsp;' . get_column('store',array('store_id'=>$val['store_id']),'store_name') . '</td>';
