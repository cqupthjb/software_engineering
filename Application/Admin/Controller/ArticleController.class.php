...
class ArticleController extends BaseController {
    ...
    public function feedbackList(){
        ...
        $where = " 1 = 1 ";
        $keywords = trim(I('keywords'));
        $keywords && $where.=" and msg_title like '%$keywords%' ";
        $cat_id = I('cat_id',0);
        $cat_id && $where.=" and cat_id = $cat_id ";
        $res = $feedback->where($where)->order('msg_id desc')->page("$p,$size")->select();
        $count = $feedback->where($where)->count();// 查询满足要求的总记录数
        $pager = new \Think\Page($count,$size);// 实例化分页类 传入总记录数和每页显示的记录数
        $page = $pager->show();//分页显示输出

        if($res){
            foreach ($res as $val){
                $val['msg_time'] = date('Y-m-d H:i:s',$val['msg_time']);
                $list[] = $val;
            }
        }
