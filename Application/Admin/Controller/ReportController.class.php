...
class ReportController extends BaseController{
    ...
    //卡券分成统计图
    public function card_rebate_log(){
        $today = strtotime(date('Y-m-d'));
        $month = strtotime(date('Y-m-01'));
        $user['today'] = M('card_rebate_log')->where("confirm_time>$today and status=3 and  admin_id=1")->sum('money');//今日分成
        $user['month'] = M('card_rebate_log')->where("confirm_time>$month and status=3 and  admin_id=1")->sum('money');//本月分成
        $user['total'] = M('card_rebate_log')->where("status=3 and  admin_id=1")->count();//分成总数
        $user['user_money'] = D('card_rebate_log')->where("status=3 and  admin_id=1")->sum('money');//分成总额

        $this->assign('user',$user);
        $sql = "SELECT sum(money) as mo,FROM_UNIXTIME(confirm_time,'%Y-%m-%d') as gap from __PREFIX__card_rebate_log where confirm_time>{$this->begin} and confirm_time<{$this->end} and status=3 and  admin_id=1 group by gap";
        $new = M()->query($sql);//新增会员趋势
        foreach ($new as $val){
            $arr[$val['gap']] = $val['mo'];
        }
    ...
    public function card_index(){
        $now = strtotime(date('Y-m-d'));
        $today['today_amount'] = M('card_order')->where("create_time>$now AND (pay_status=1 or pay_code='cod') and order_status in(1,2,4)")->sum('total_pay');//今日销售总额
        $today['today_order'] = M('card_order')->where("create_time>$now and (pay_status=1 or pay_code='cod')")->count();//今日订单数
        $today['cancel_order'] = M('card_order')->where("create_time>$now AND order_status=3")->count();//今日取消订单
        $today['sign'] = round($today['today_amount']/$today['today_order'],2);
        $this->assign('today',$today);
        $sql = "SELECT COUNT(*) as tnum,sum(total_pay) as amount, FROM_UNIXTIME(create_time,'%Y-%m-%d') as gap from  __PREFIX__card_order ";
        $sql .= " where create_time>$this->begin and create_time<$this->end AND (pay_status=1 or pay_code='cod') and order_status in(1,2,4) group by gap ";
        $res = M()->query($sql);//订单数,交易额
    ...
    //卡券销售排行
    public function card_saleTop(){
        $sql = "select card_name,card_id,sum(card_num) as sale_num,sum(card_num*(card_price-default_rebate)) as sale_amount from __PREFIX__card_suborder ";
        $sql .=" where is_send = 1 group by card_id order by sale_amount DESC limit 100";
        $res = M()->cache(true,3600)->query($sql);
        $this->assign('list',$res);
        $this->display();
    }
