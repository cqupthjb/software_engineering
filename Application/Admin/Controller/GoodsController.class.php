...
class GoodsController extends BaseController {
    ...
    public function goodsList(){      
        ...
        $storeList = M('store')->where(array('role_id'=>array('in','0,5')))->field('store_id,store_name')->select();
    ...
    public function addEditGoods()
    {
        ...
        $GoodsLogic = new \Seller\Logic\GoodsLogic();
        $Goods = D('Admin/Goods'); //
        ...
        try{
            if (($_GET['is_ajax'] == 1) && IS_POST) {
                ...
                $data = $this->_check_goods($type);

                if ($data===false)// 根据表单提交的POST数据创建数据对象
                {
                    //  编辑
                    ...
                    if ($Goods->distribut > ($Goods->shop_price / 2))
                        throw new Exception( '分销的分成金额不得超过商品金额的50',-1);
                    ...
                    if ($type == 2) {
                        ...
                        $goods_id = $Goods->add($data); // 新增数据到数据库
                        if ($goods_id === false)
        ...
        $shipping_area = D('Seller/shipping_area')->getShippingArea($store_id);//配送区域
        $goods_shipping_area_ids = explode(',', $goodsInfo['shipping_area_ids']);
        $rebateList = M('Rebate')->order('value asc')->select();//返利比例  不明觉厉
    ...
    //检测商品数据
    protected function _check_goods($type){
        $Goods = D('Admin/Goods');
        $data = $Goods->create(I('post.'), $type);

        //模型数据验证  同意验证  没啥用
        if ($data === false)
            throw new Exception('请重新检测商品数据是否填写完整！',-1);


        //自定义数据验证
        if (empty($data['cat_id1']) || empty($data['cat_id2']) || empty($data['cat_id3'])){
            throw new Exception('请重新选择商品分类',-1);
        }
        if (empty($data['goods_name'])){
            throw new Exception('请填写商品名称',-1);
        }
        if (empty($data['original_img'])){
            throw new Exception('请上传商品图片',-1);
        }
        if (empty($data['goods_content'])){
            throw new Exception('请填写商品详情',-1);
        }

        /*$img = 'http://'.$_SERVER['HTTP_HOST'].$data['original_img'];
        if (!is_file($img)){
            throw new Exception('请重新上传图片');
        }*/
    ...
    public function ajaxGetSpecSelect(){
        ...
        $map = array('id'=>array('in',$spec_id_arr));
        $specList = M('Spec')->where($map)->order('`order` desc')->select(); // 找出这个类型的所有规格
        foreach ($specList as $k => $v)
            $specList[$k]['spec_item'] = M('SpecItem')->where("store_id = " . $store_id . " and spec_id = " . $v['id'])->getField('id,item'); // 获取规格项
