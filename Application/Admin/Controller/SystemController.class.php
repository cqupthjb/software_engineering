...
class SystemController extends BaseController{
	...
	{          
		/*配置列表*/
		$group_list = array('shop_info'=>'网站信息','basic'=>'基本设置','shopping'=>'购物流程设置','water'=>'水印设置','distribut'=>'分销设置','wholesaler'=>'批发商设置',"withdraw_setting"=>"提现设置",'about_me'=>'关于我们');// 'sms'=>'短信设置','smtp'=>'邮件设置',
		$this->assign('group_list',$group_list);
		$inc_type =  I('get.inc_type','shop_info');
		$this->assign('inc_type',$inc_type);
        if ($inc_type == 'about_me'){
            $this->initEditor();//编辑器
        }
	...
	public function cleanCache(){
		...
		if(IS_POST)
		{
            ...
            in_array('qr_code',$_POST['clear']) && delFile('./Public/upload/qrcode');//生成的二维码图片
            delFile('./Public/upload/ad/thumb');//广告缩略图
    ...
    {
        $this->assign("URL_upload", U('Admin/Ueditor/imageUp',array('savepath'=>'sys'))); // 图片上传目录
        $this->assign("URL_imageUp", U('Admin/Ueditor/imageUp',array('savepath'=>'sys'))); //  不知道啥图片
        $this->assign("URL_fileUp", U('Admin/Ueditor/fileUp',array('savepath'=>'sys'))); // 文件上传s
        $this->assign("URL_scrawlUp", U('Admin/Ueditor/scrawlUp',array('savepath'=>'sys')));  //  图片流
        $this->assign("URL_getRemoteImage", U('Admin/Ueditor/getRemoteImage',array('savepath'=>'sys'))); // 远程图片管理
        $this->assign("URL_imageManager", U('Admin/Ueditor/imageManager',array('savepath'=>'sys'))); // 图片管理
        $this->assign("URL_getMovie", U('Admin/Ueditor/getMovie',array('savepath'=>'sys'))); // 视频上传
